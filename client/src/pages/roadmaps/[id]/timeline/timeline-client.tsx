...
import { JiraIssueDetailPanel } from "../../../../components/jira-issue-detail-panel";
...
import { useJiraIssuePreview } from "../../../../hooks/use-jira-issue-preview";
import {
  buildTicks,
  buildYearSpans,
...
export type TimelineSegment = {
  ...
  /** Initiative `sourceReference` or phase `jiraKey` for Jira hover preview. */
  jiraDisplay: string;
};

/** Sticky theme/group column — keep narrow so the Gantt area gets horizontal space. */
const LABEL_COL_MAX_PX = 200;
const LABEL_COL_CLASS = "min-w-[140px] max-w-[200px] w-[min(18vw,200px)] shrink-0";
  ...
  for (const item of items) {
    ...
    if (item.phases.length === 0) {
      ...
      out.push({
        ...
        groupTeam: team,
        themeColorToken,
        jiraDisplay: item.initiative.sourceReference?.trim() ?? "",
        itemForm: {
...
function TimelineBarTooltip({ row }: { row: TimelineSegment }) {
  const jiraPreview = useJiraIssuePreview(row.jiraDisplay, true);
  return (
    <div
    ...
    >
      ...
      {row.jiraDisplay.trim() ? (
        <div className="mt-2 border-t border-slate-700/80 pt-2">
          <p className="mb-1.5 text-[10px] font-medium uppercase tracking-wide text-slate-500">Jira</p>
          <JiraIssueDetailPanel
            value={row.jiraDisplay}
            loading={jiraPreview.loading}
            err={jiraPreview.err}
            issue={jiraPreview.issue}
            browseBase={jiraPreview.browseBase}
            hasConfig={jiraPreview.hasConfig}
            compact
          />
        </div>
      ) : null}
...
export function TimelineClient({
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="min-w-0" style={{ minWidth: chartMinWidth + LABEL_COL_MAX_PX + 24 }}>
          {segments.length === 0 && (
            <div className="border-b border-slate-700 px-4 py-10 text-center text-sm text-slate-400">
              ...
              <Link to={`/roadmaps/${roadmapId}`} className="text-indigo-400 hover:text-indigo-300">
                {" "}
                grid view
              </Link>
              .
                ...
                <div className="relative h-full w-full">
                  ...
                  {g.rows.map((row) => {
                    ...
                    return (
                      ...
                      >
                        <Link
                          to={`/initiatives/${row.initiativeId}`}
                ...
                {workspacePhases.length > 0 ? (
                  <>
                    ...
                    <p className="text-xs text-slate-500">
                      ...
                      <Link to="/phases" className="text-indigo-400 hover:text-indigo-300">
                        Phases
                      </Link>
                      .
                    </p>
