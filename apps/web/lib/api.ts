...
export async function loadJson<T>(path: string): Promise<LoadJsonResult<T>> {
  try {
    ...
    return {
      ok: false,
      message: `Cannot reach ${API_BASE}${path} (${detail}). Start **api-gateway** on port 4000 (e.g. \`npm run dev\` or \`npm run dev -w @roadmap/api-gateway\`).`,
    };
  }
}
