          ...
          <section className={themeDetailObjectiveSectionClass(t.colorToken ?? null)}>
            ...
            {t.roadmap ? (
              <p className="mt-4 text-sm text-slate-500">
                Scoped to roadmap{" "}
                <Link href={`/roadmaps/${t.roadmap.id}`} className="text-indigo-300 hover:text-indigo-200">
                  {t.roadmap.name}
                </Link>{" "}
            ...
            <ul className="mt-4 divide-y divide-slate-800">
              ...
              {sorted.map((row) => (
                <li key={row.initiative.id} className="py-4 first:pt-0">
                  <Link
                    href={`/initiatives/${row.initiative.id}`}
                    className="font-medium text-indigo-300 hover:text-indigo-200"
                  >
                    {row.initiative.canonicalName}
                  </Link>
                  {row.initiative.shortObjective?.trim() ? (
                    <p className="mt-2 text-sm text-slate-400 whitespace-pre-wrap line-clamp-4">
