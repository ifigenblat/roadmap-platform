    ...
    <>
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          {(data.type?.trim() || data.sourceReference?.trim()) ? (
            <p className="mt-1 text-sm text-slate-500">
              {[data.type?.trim(), data.sourceReference?.trim()].filter(Boolean).join(" · ")}
            </p>
          ) : null}
            ...
            <ul className="mt-3 list-disc space-y-2 pl-5 text-slate-200">
              {(data.themes ?? []).map((lt) => (
                <li key={lt.strategicTheme.id}>
                  <Link href={`/themes/${lt.strategicTheme.id}`} className="text-indigo-300 hover:text-indigo-200">
            ...
            <ul className="mt-4 space-y-4 text-slate-200">
              {(data.roadmapItems ?? []).map((ri) => (
                <li key={ri.id} className="border-b border-slate-800/80 pb-4 last:border-b-0 last:pb-0">
                  <div>
                    <Link href={`/roadmaps/${ri.roadmap.id}`} className="text-indigo-300 hover:text-indigo-200">
                      {ri.roadmap.name}
                    </Link>
